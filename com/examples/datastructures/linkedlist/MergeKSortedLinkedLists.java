...
public class MergeKSortedLinkedLists {
	...
	public static Node mergeKLists(Node[] list, int k) {
		...
		pq = new PriorityQueue<Node>(Comparator.comparingLong(a -> ((Node) a).data));
	...
	public static void main(String[] s) {
		...
		list[0] = new Node(1L);
		list[0].next = new Node(5L);
		list[0].next.next = new Node(7L);

		list[1] = new Node(2L);
		list[1].next = new Node(3L);
		list[1].next.next = new Node(6L);
		list[1].next.next.next = new Node(9L);

		list[2] = new Node(4L);
		list[2].next = new Node(8L);
		list[2].next.next = new Node(10L);
